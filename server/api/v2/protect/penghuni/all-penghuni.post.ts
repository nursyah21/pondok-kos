export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        let data = await Users.find({ role: 0 })
            .select(['name', 'number_phone','avatar','id_card','address','verified','birthdate'])
            .skip(Number(skip))
        .limit(Number(limit))
        
        // @ts-ignore
        data = data.map((e,idx)=>{
            return {num:Number(skip)+idx+1, id:e._id, name:e.name, number_phone: e.number_phone, avatar: e.avatar, 
                id_card: e.id_card, address: e.address, birthdate: e.birthdate, verified: e.verified }
        })
