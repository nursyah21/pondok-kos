export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const check = await ResetPassword.find({
      id_user: exist._id,
      createdAt: {
    ...
    if (check.length) {
      ...
      if (check)
        return {
          status: "success",
          message: `reset password sudah dikirim coba lagi setelah ${differentTime(
            check[0].createdAt
          )}`,
        };
    }
    ...
    await $fetch("/api/fontee", {
      ...
      body: {
        target: number_phone,
        message: `Kami menerima permintaan untuk mereset kata sandi akun Kos Pondok Jaya Anda. Untuk mereset kata sandi Anda, silakan klik tautan berikut:
