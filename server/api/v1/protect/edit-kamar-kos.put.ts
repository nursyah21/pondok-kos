export default defineEventHandler(async (event) => {
    const { token, _id, id_kos, name, description, available:_available, image } = await readBody(event)
    ...
    try {
        ...
        const res = await KamarKos.findByIdAndUpdate(_id, {id_kos, name, description, available, image})
