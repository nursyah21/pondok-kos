...
<template>
    <UCard  v-if="status == 'success'" class="max-h-[300px]" :ui="{ header: { padding: 'py-2' }, body: {padding:'py-0'} }">
        ...
        <apexchart :options="props.optionsChart" :series="props.series" class="dark:text-yellow-400"
        ></apexchart>
        <template #footer>
            <h1 v-if="description" class="text-xs text-slate-600 dark:text-slate-200">
                {{ props.description }}
            </h1>
        </template>
    </UCard>
